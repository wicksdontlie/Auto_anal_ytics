...
def _is_datetime_dtype(dtype) -> bool:
    """
    Safely check if a dtype is datetime64.
    Handles pandas extension types (StringDtype, Int64Dtype, etc.)
    that np.issubdtype cannot handle.
    """
    try:
...
def _detect_column_types(df: pd.DataFrame):
    ...
    for c in df.columns:
        if _is_datetime_dtype(df[c].dtype):
            datetime.append(c)
        else:
...
def ask_llm(prompt: str, df: pd.DataFrame,
    ...
    Analyze the provided dataset carefully.

    Use statistics, patterns, missing values,
    ...
    Answer professionally and analytically.
    """
...
def build_dataset_context(df: pd.DataFrame) -> str:
    ...
    return context
def is_code_request(prompt: str):
    ...
    return any(word in prompt for word in code_keywords)
def is_code_request(prompt: str):
